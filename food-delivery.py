...
def validate_beverage(slots):
    ...
    if location and not isvalid_city( location ):
        return build_validation_result(
            False,
            'Location',
            'We currently do not support {} as a valid delivery address.  Can you try a different city?'.format(
                location )
        )
...
def validate_snacks(slots):
    ...
    if snack and not isvalid_snack( snack ):
        return build_validation_result( False, 'Fast',
                                        'I did not recognize that Fast food.  Do you want to have pizza, fries ,burger?' )
...
def order_snacks(intent_request):
    ...
    pr = 'Order Id : {}\nYour order for {} {} of {} dollars at {} has been placed and it will be delivered within 30 mins.\nThanks for ordering your meal.\n Have a nice day.'.format(
        recordId, quantity, snacks, quantity * 11, location )
...
def order_beverages(intent_request):
        ...
        if confirmation_status == 'None':
            ...
            if (not location and not quantity and not drink) \
                ...
                if last_confirmed_reservation and try_ex(
                        lambda: last_confirmed_reservation['ReservationType'] ) == 'Snacks':
    ...
    email_body = 'Body'
    pr = 'Order Id : {}\nYour order for {} {} of {} dollars at {} has been placed and it will be delivered within 30 mins.\nThanks for ordering your meal.\n Have a nice day.'.format(
        recordId, quantity, drink, quantity * 10, location )
